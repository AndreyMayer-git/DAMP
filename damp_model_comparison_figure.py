"""
Figure 6 + power analysis for the DAMP manuscript: the two competing generative
models bracket the sign of DCI, and a simulation-based power analysis (Wilson &
Collins, 2019). No human data is involved.

Panel A: DCI as a function of the salience-capture weight gamma in the LCA
  (Model 2). The minimal DDM (Model 1) is shown as a horizontal baseline. The two
  models make OPPOSITE predictions: DDM -> DCI <= 0; LCA -> DCI > 0 once salience-
  weighted competition is present. Empirical data are needed to arbitrate.
Panel B: an example high-conflict trajectory produced by the LCA dynamics
  themselves (early excursion toward the aversive option, then correction) -- in
  contrast to Figure 3, this hook is generated by the model, not injected.
"""
